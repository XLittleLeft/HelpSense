...
namespace HelpSense.Commands
{
    ...
    public class InfoCommand : ICommand
    {
        ...
        public string[] Aliases => new string[] { "info" };

        public string Description => "查询在本服务器游玩的时间和击杀人数";

        public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response)
        {
            var pSender = Player.Get(sender);
            if (pSender != null && !pSender.DoNotTrack && Plugin.Instance.Config.SavePlayersInfo)
            {
                var Log = pSender.GetLog();
                ...
                pSender.SendConsoleMessage($"\n自从插件安装后你已在本服游玩了" +
                    $"\n<color=red>{day}</color>天<color=red>{hour}</color>小时<color=red>{minutes}</color>分钟" +
